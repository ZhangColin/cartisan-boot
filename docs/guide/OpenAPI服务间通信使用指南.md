...
| 问题 | 解决方案 |
|------|----------|
| 服务间调用身份不明 | apiKey/apiSecret 签名机制 |
...
**自动行为**：
- 自动计算 HMAC-SHA256 签名
- 自动添加签名 headers（X-Api-Key, X-Timestamp, X-Nonce, X-Body-Digest, X-Sign）
...
| 字段 | 来源 | 获取方式 |
|------|------|----------|
| callerAppId | X-Api-Key header | `RequestContext.getCallerAppId()` |
...
| 自定义 | 实现 `ApiKeyProvider.getByApiKey(apiKey)` |
