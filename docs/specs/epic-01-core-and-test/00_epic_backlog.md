...
**描述：**
基于 Epic 1 完成后的审计反馈，补充 CLAUDE.md、README.md、PIT 门禁脚本等文档，确保流程规范可执行。

**验收标准：**
- [ ] CLAUDE.md 新增「开发环境要求」和「质量门禁」小节
- [ ] 项目根目录创建 README.md，包含快速开始和环境说明
- [ ] 创建 `scripts/run-pitest.sh` 脚本，支持模块化 PIT 执行
- [ ] SKILL.md 新增 TOOL-004（PIT 执行说明）
- [ ] Docker 环境依赖在文档中有清晰说明
- [ ] 文档间引用关系正确

**技术要点：**
- README.md 作为项目入口，链接到详细文档
- PIT 脚本支持模块参数，输出报告路径提示
- Docker 依赖说明区分单元测试和集成测试的运行方式
